    ...
    <script>
        ...
        function printOrder(orderId) {
            ...
            printWin.document.write('<!DOCTYPE html><html><head><title>Receipt ' + order.code + '</title>' +
                ...
                '.footer{text-align:center;color:#999;margin-top:16px;font-size:10px;}</style></head><body>' +
                '<h2>{{ Auth::user()->name }}</h2>' +
                '<p class="sub">Order Receipt</p><hr>' +
                '<p><strong>' + order.code + '</strong></p>' +
                '<p>Customer: ' + order.customer + '</p>' +
                '<p>Date: ' + order.deliveryDate + '</p>' +
                '<p>Location: ' + order.location + '</p><hr>' +
        ...
        for (var i = 0; i < rangeTabs.length; i++) {
            rangeTabs[i].addEventListener('click', function() {
                window.location.href = '/dashboard?range=' + this.getAttribute('data-range');
            });
        }
